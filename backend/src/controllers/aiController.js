...
try {
  ...
  console.log('[aiController] OpenAI package unavailable.');
}
...
async function handleChat(req, res) {
  try {
    ...
    let retrievedText = '';
    if (generateEmbedding && process.env.DATABASE_URL) {
      try {
        const queryEmbedding = await generateEmbedding(message.trim());
        const queryVector = `[${queryEmbedding.join(',')}]`;
        const result = await db.query(
          `SELECT text FROM "KnowledgeDocument" ORDER BY embedding <-> $1::vector LIMIT 2`,
          [queryVector]
        );
        if (result.rows && result.rows.length > 0) {
          retrievedText = result.rows.map(doc => doc.text).join('\n---\n');
        }
      } catch (dbErr) {
        console.error("RAG search failed (non-critical):", dbErr.message);
      }
    }
    ...
    if (process.env.DATABASE_URL) {
      try {
        const result = await db.query(
          'SELECT name, breed FROM "Pet" WHERE "userId" = $1',
          [userId]
        );
        if (result.rows.length) {
          userPetsStr = result.rows.map(p => p.name + ' (' + p.breed + ')').join(', ');
        }
      } catch {
    ...
    const systemPrompt = {
      role: "system",
      content: `You are Cuddles, an expert vet AI. You MUST base your answer strictly on this official company knowledge: [${retrievedText || 'No internal knowledge retrieved'}]. If the knowledge base mentions a specific policy, quote it directly. The user's pets are: [${userPetsStr}].`,
    };
